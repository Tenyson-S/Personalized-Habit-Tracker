    ...
    <Modal visible={visible} animationType="slide" presentationStyle="fullScreen" onRequestClose={close}>
      <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
        {dashboard.isLoading ? <View style={styles.center}><ActivityIndicator /></View> : !dashboard.data ? (
          ...
          <HabitDetail habit={selected} onBack={() => setSelectedId(null)} onClose={close} styles={styles} colors={colors} />
        ) : (
          <DashboardOverview data={dashboard.data} onSelect={setSelectedId} onClose={close} styles={styles} />
        )}
      </SafeAreaView>
    </Modal>
  );
...
function DashboardOverview({ data, onSelect, onClose, styles }: { data: HabitDashboardPayload; onSelect: (id: string) => void; onClose: () => void; styles: any }) {
          ...
          {focus.foundation.required && !focus.foundation.established ? (
            ...
            <>
              ...
              <View style={styles.statRow}>
                <MiniStat label="consistency" value={`${Math.round(focus.metrics.consistency_30_days)}%`} styles={styles} />
                <MiniStat label="perfect run" value={`${focus.metrics.perfect_run}`} styles={styles} />
                <MiniStat label="returns" value={`${focus.metrics.returns}`} styles={styles} />
              </View>
            </>
          )}
        </Pressable>
...
function HabitDetail({ habit, onBack, onClose, styles, colors }: { habit: HabitDashboardItem; onBack: () => void; onClose: () => void; styles: any; colors: ThemeColors }) {
  return (
    <ScrollView contentContainerStyle={styles.page}>
      ...
      <View style={styles.detailStats}>
        <DetailStat label="consistency" value={`${Math.round(habit.metrics.consistency_30_days)}%`} detail="last 30 days" styles={styles} />
        <DetailStat label="perfect run" value={`${habit.metrics.perfect_run}`} detail="scheduled check-ins" styles={styles} />
        <DetailStat label="returns" value={`${habit.metrics.returns}`} detail="after quiet gaps" styles={styles} />
      </View>
        ...
        <View style={styles.calendarGrid}>
          {habit.history.map((day, index) => (
            <View key={day.date} style={styles.calendarCell}>
              <View style={[styles.dayDot, historyStyle(day.status, colors), index === habit.history.length - 1 && styles.todayDot]} />
              <Text style={styles.dayNumber}>{Number(day.date.slice(-2))}</Text>
            </View>
          ))}
        </View>
        <View style={styles.legendRow}>
          <LegendDot color={colors.text} label="done" styles={styles} colors={colors} />
          <LegendDot color={colors.primary} label="scheduled" styles={styles} colors={colors} />
          <LegendDot color="transparent" border label="rest" styles={styles} colors={colors} />
        </View>
      </View>
...
function historyStyle(status: HabitHistoryStatus, colors: ThemeColors) {
  if (status === 'COMPLETE') return { backgroundColor: colors.text, borderColor: colors.text };
  if (status === 'MISSED') return { backgroundColor: colors.surfaceMuted, borderColor: colors.surfaceMuted };
  if (status === 'OPEN') return { backgroundColor: colors.primarySoft, borderColor: colors.primary };
...
function DetailStat({ label, value, detail, styles }: { label: string; value: string; detail: string; styles: any }) {
...
function LegendDot({ color, label, border = false, styles, colors }: { color: string; label: string; border?: boolean; styles: any; colors: ThemeColors }) {
...
const useStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  heroTitle: { color: colors.text, fontSize: 34, lineHeight: 38, fontWeight: '700', letterSpacing: -1, marginTop: 4 },
  roundButton: { width: 44, height: 44, borderRadius: 99, borderWidth: 1, borderColor: colors.text, alignItems: 'center', justifyContent: 'center' },
  roundButtonText: { color: colors.text, fontSize: 23, lineHeight: 25 },
  ...
  metricLabelLight: { color: colors.textMuted, fontSize: 10, letterSpacing: 0.5, textTransform: 'uppercase', fontWeight: '600' },
  metricValue: { color: colors.text, fontSize: 32, fontWeight: '700', letterSpacing: -1 },
  metricValueLight: { color: colors.text, fontSize: 32, fontWeight: '700', letterSpacing: -1 },
  metricUnit: { color: colors.textMuted, fontSize: 11 },
  metricUnitLight: { color: colors.textMuted, fontSize: 11 },
  focusCard: { backgroundColor: colors.primarySoft, borderRadius: radius.xl, padding: 20, gap: 10, minHeight: 180 },
  focusTop: { flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 },
  focusName: { color: colors.text, fontSize: 24, lineHeight: 28, fontWeight: '600', letterSpacing: -0.5 },
  focusMeta: { color: colors.textMuted, marginTop: 4, fontSize: 12 },
  arrow: { color: colors.text, fontSize: 24 },
  focusBig: { color: colors.text, fontSize: 36, lineHeight: 42, fontWeight: '700', letterSpacing: -1.5, marginTop: 4 },
  focusCaption: { color: colors.text, fontSize: 14 },
  track: { height: 7, borderRadius: 99, backgroundColor: colors.surfaceMuted, overflow: 'hidden' },
  ...
  miniValue: { color: colors.text, fontSize: 19, fontWeight: '700', marginTop: 2 },
  ...
  sectionTitle: { color: colors.text, fontSize: 22, lineHeight: 26, fontWeight: '600', letterSpacing: -0.5, marginTop: 2 },
  sectionCount: { color: colors.text, fontSize: 18, fontWeight: '600' },
  habitGrid: { gap: 10 },
  habitTile: { borderRadius: radius.lg, padding: 18, minHeight: 110, justifyContent: 'space-between' },
  mintTile: { backgroundColor: colors.primarySoft },
  creamTile: { backgroundColor: colors.surface },
  greenTile: { backgroundColor: colors.surfaceMuted },
  habitName: { color: colors.text, fontSize: 18, lineHeight: 22, fontWeight: '600' },
  ...
  arrowSmall: { color: colors.text, fontSize: 18 },
  ...
  noteTitle: { color: colors.text, fontSize: 23, lineHeight: 28, fontWeight: '600' },
  ...
  detailHeaderTitle: { color: colors.text, fontSize: 11, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 1 },
  rule: { height: 1, backgroundColor: colors.text, opacity: 0.25 },
  detailName: { color: colors.text, fontSize: 34, lineHeight: 38, fontWeight: '700', letterSpacing: -1, marginTop: 10 },
  detailSchedule: { color: colors.textMuted, fontSize: 13, marginTop: 4 },
  persistenceHero: { minHeight: 180, backgroundColor: colors.primarySoft, borderRadius: radius.xl, padding: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  persistenceNumber: { color: colors.text, fontSize: 64, lineHeight: 68, fontWeight: '700', letterSpacing: -3 },
  persistenceCaption: { color: colors.text, fontSize: 15 },
  ...
  sideValue: { color: colors.text, fontSize: 24, fontWeight: '700' },
  ...
  detailStatValue: { color: colors.text, fontSize: 28, fontWeight: '600', marginVertical: 8 },
  foundationCard: { backgroundColor: colors.surfaceMuted, borderRadius: radius.xl, padding: 20, gap: 12 },
  existingCard: { backgroundColor: colors.primarySoft, borderRadius: radius.xl, padding: 20, gap: 10 },
  foundationTitle: { color: colors.text, fontSize: 25, lineHeight: 29, fontWeight: '600', marginTop: 4 },
  foundationPercent: { color: colors.text, fontSize: 24 },
  ...
  historyTitle: { color: colors.text, fontSize: 28, fontWeight: '500' },
  ...
  todayDot: { borderWidth: 2, borderColor: colors.text },
