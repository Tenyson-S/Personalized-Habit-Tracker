...
export function ActivityManager({onChanged, onEdit}:{onChanged:()=>void, onEdit:(type:'habits'|'dailies'|'tasks', item:any)=>void}) {
  ...
  return (
    <Card>
      ...
      <Text style={styles.muted}>Create, review, and remove what no longer belongs.</Text>
      {sections.map(([label, items, type]) => (
        <View key={label} style={styles.section}>
          <Text style={styles.label}>{label}</Text>
          {items.length === 0 ? <Text style={styles.muted}>None yet.</Text> : items.slice(0,8).map((item:any) => (
            <View key={item.id} style={styles.row}>
              <View style={{flex:1}}>
                <Text style={styles.item}>{item.name??item.title}</Text>
                <Text style={styles.meta}>{(item.life_area??'').replaceAll('_',' ')} • {item.preferred_time?.slice(0,5) || item.due_date || 'No schedule'}</Text>
              </View>
              <View style={styles.actions}>
                <Pressable onPress={()=>onEdit(type,item)}>
                  <Text style={styles.edit}>Edit</Text>
                </Pressable>
                <Pressable onPress={()=>changeState(type,item)}>
                  <Text style={styles.pause}>{type==='tasks'?'Archive':item.status==='PAUSED'?'Resume':'Pause'}</Text>
                </Pressable>
                <Pressable onPress={()=>remove(type,item.id,item.name??item.title)}>
